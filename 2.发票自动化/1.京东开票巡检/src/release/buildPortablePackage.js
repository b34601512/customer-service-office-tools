...
function 获取分发目录名(显示版本) {
  ...
  return `jd-invoice-watch-cs-v${显示版本}`;
}
...
function 构建干净数据目录(分发目录) {
  ...
  写入JSON文件(path.join(数据目录, 'store-results.json'), { stores: {} });
  写入JSON文件(path.join(数据目录, 'latest-invoices.json'), []);
  写入JSON文件(path.join(数据目录, 'invoice-order-state.json'), {
    version: 2,
    orders: {},
...
function 构建干净运行目录(分发目录) {
  // 解决：分发包自带 node.exe，但不携带当前电脑的浏览器登录档案。
...
function 获取分发根文件列表() {
  ...
  return ['启动巡检.bat', 'package.json', 'package-lock.json', 'README.md', '打包配置.json'];
}
