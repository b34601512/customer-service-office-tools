...
function 创建TUI(选项 = {}) {
  return 创建回传平台TUI({
    ...
    订单页扩展列: [
      ...
      { 标题: "赔付倒计时", 宽度: 16, 取值: (镜像) => 镜像.原订单?.compensationCountdown || "-" },
      { 标题: "是否已批准", 宽度: 10, 取值: (镜像) => 镜像.原订单?.approvalStatus || "-", 颜色: (镜像) => 镜像.原订单?.approvalStatus === "已批准" ? "brightGreen" : "yellow" },
      { 标题: "发票抬头", 宽度: 20, 取值: (镜像) => 镜像.原订单?.invoiceTitle || "-" },
