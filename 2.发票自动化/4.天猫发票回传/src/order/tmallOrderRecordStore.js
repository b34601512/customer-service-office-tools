// 该文件用于把天猫只读订单同步到平台独立 JSON，并向 CLI 与正式回传提供同一份四阶段订单事实。
...
const 天猫订单记录文件路径 = path.join(数据目录, 'invoice-order-records.json');

function 构建天猫订单快照(store, order, now) {
  const operationStatus = String(order.operationStatus || '未识别操作').trim();
  return {
    source: '天猫待回传列表只读同步',
    platformStatus: {
      kind: operationStatus === '可录入发票'
        ? 'returnable'
        : (operationStatus === '待同意' ? 'pending_agree' : 'unknown'),
...
const 服务 = 创建平台订单状态服务({
  platformName: '天猫',
  projectName: '4.天猫发票回传',
  defaultFilePath: 天猫订单记录文件路径,
  projectRoot: 项目根目录,
  buildSnapshot: 构建天猫订单快照,
});

function 同步天猫待处理订单({ store = {}, orders = [] } = {}, filePath = 天猫订单记录文件路径) {
  // 天猫待回传列表是后台事实快照：本次后台不存在的本店旧订单必须移除，避免再次进入回传队列。
  ...
  for (const [key, record] of Object.entries(data.orders || {})) {
    if (String(record.storeId || '').trim() === storeId
      && !incomingNumbers.has(String(record.orderNumber || '').trim())) {
