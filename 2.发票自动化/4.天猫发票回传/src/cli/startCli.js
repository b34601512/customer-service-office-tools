...
const { 登录单个天猫店铺, 登录全部启用天猫店铺 } = require('../app/loginStores');
const { 采集单个天猫店铺元素 } = require('../app/collectTmallElements');
const { 同步天猫待处理订单 } = require('../app/syncPendingOrders');
const { 执行天猫发票正式回传 } = require('../app/returnInvoiceToTmall');
const {
  读取订单列表,
  读取店铺发票已登记订单,
  更新订单工作流状态,
...
const 店铺字段定义 = [
  ...
  { 键: 'targetUrl', 名称: '登录地址', 默认值: 天猫默认登录地址, 必填: true },
  { 键: 'username', 名称: '登录账号' },
...
async function 采集指定店铺({ 提问器, 输出, 终端 }) {
  const 店铺 = (await 选择店铺({ 提问器, 店铺列表: 获取启用店铺列表(), 允许全部: false, 输出, 终端 }))[0];
  const 结果 = await 采集单个天猫店铺元素(店铺, { headless: false });
