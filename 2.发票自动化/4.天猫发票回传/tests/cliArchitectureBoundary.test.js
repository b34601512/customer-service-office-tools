// 该文件用于防止天猫回传重新引入已迁出的网页控制台，并锁定 CLI 与通用清理边界。
...
test('CLI架构边界：启动清理继续覆盖天猫导出', () => {
  const 路径列表 = 构建启动清理路径列表(path.join(项目根目录, '边界测试'));
  assert.equal(路径列表.some((项目) => 项目.includes(path.join('runtime', 'tmall-exports'))), true);
});
...
test('CLI架构边界：平台业务入口保留', () => {
  assert.equal(fs.existsSync(path.join(项目根目录, 'src/main.js')), true);
});
