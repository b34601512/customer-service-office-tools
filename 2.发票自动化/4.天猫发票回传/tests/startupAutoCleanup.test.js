...
function 创建临时项目目录() {
  ...
  return fs.mkdtempSync(path.join(os.tmpdir(), 'tmall-runtime-cleanup-'));
}

test('启动清理路径不包含登录资料和旧控制台目录', () => {
  const projectRoot = path.join(os.tmpdir(), 'tmall-project');
  const paths = 构建启动清理路径列表(projectRoot);

  assert.equal(paths.some((item) => item.includes(path.join('runtime', 'store-profiles'))), false);
  assert.equal(paths.some((item) => item.includes(path.join('runtime', 'tmall-exports'))), true);
...
test('启动清理会迁移临时产物但保留店铺资料', async () => {
  ...
  const 导出目录 = path.join(projectRoot, 'runtime', 'tmall-exports');
  ...
  assert.match(result[0].备份路径, /天猫发票回传-runtime缓存备份/);
});
