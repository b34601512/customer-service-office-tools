...
const 店铺字段定义 = [
  ...
  { 键: 'phoneNumber', 名称: '登录手机号', 必填: true },
  { 键: 'platformStoreId', 名称: '平台店铺ID', 必填: true },
  { 键: 'platformStoreName', 名称: '平台完整店名', 必填: true },
...
async function 发票回传({ 提问器, 输出, 终端, 记录运行日志 }) {
  const stores = await 选择店铺({ 提问器, 店铺列表: 获取启用店铺列表(), 允许全部: true, 输出, 终端 });
  const result = await 逐店同步并回传({
    stores,
    onProgress: (progress) => { if (typeof 记录运行日志 === 'function') 记录运行日志(progress.message); },
  });
  终端.显示页面('抖音 · 回传结果');
  输出(result.message);
  return result;
