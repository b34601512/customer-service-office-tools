...
const { 登录首个或指定抖音店铺, 登录全部启用抖音店铺 } = require('./app/loginStores');
const { 采集首个或指定抖音店铺元素 } = require('./app/collectDouyinElements');
...
async function main() {
  ...
  if (模式 === 'login') {
    ...
    await 登录首个或指定抖音店铺(storeId, { headless: false });
    return;
  }
  if (模式 === 'login-all') {
    await 登录全部启用抖音店铺({ headless: false });
    return;
  }
  if (模式 === 'collect') {
    const storeId = process.argv[3] || '';
    await 采集首个或指定抖音店铺元素(storeId, { headless: false });
  }
}
