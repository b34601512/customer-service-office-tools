...
async function 读取抖音订单快照(page, 店铺配置, dependencies) {
  ...
  try {
    ...
    const orders = dependencies.读取订单报表(exportFilePath, 店铺配置);
    if (!Array.isArray(orders) || !orders.length) throw new Error('页面存在待回传订单，但导出报表未解析出订单。');
    return {
      orders,
      exportFilePath,
      warning: '',
    ...
    throw new Error(`完整报表读取失败，未更新本地订单：${error.message}`);
  }
}
...
async function 同步抖音待处理订单(options = {}) {
  ...
  try {
    ...
    page = await dependencies.打开待回传页面(page, 店铺配置);
