...
const 抖音回传导出目录 = path.join(运行目录, 'douyin-exports');
const 下载中心常见返回秒数 = 30;
...
function 构建无待回传订单结果(店铺配置, message) {
  // 解决：当前店铺没有待开票数据是正常业务结果，不应该显示成任务失败。
  return {
    message,
    exportFilePath: '',
    totalCount: 0,
...
function 合并下载中心发票字段(order, download) {
  // 解决：下载中心只负责提供发票文件，号码字段保留给报告展示和兼容其它平台。
  return {
    ...order,
...
async function 上传已下载抖音发票(page, downloads, 选项 = {}) {
  ...
  for (const [index, item] of downloads.entries()) {
    ...
    try {
      ...
      通知回传进度(onProgress, {
        ...
        message: submit ? '抖音发票已确认回传。' : '抖音发票已上传到抽屉，未点击提交。',
        item: {
          ...构建回传报告订单(item),
...
async function 执行抖音发票回传(选项 = {}) {
  ...
  try {
    ...
    if (!使用本地已登记订单) {
      try {
        exportFilePath = await 执行带持续进度反馈({
          onProgress,
          action: () => 导出抖音待回传订单(page, 抖音回传导出目录, {
            onAction: (message) => 通知回传进度(onProgress, 构建阶段进度(message)),
          }),
          buildProgress: (等待秒数) => 构建阶段进度(`正在导出抖音待回传订单，已等待 ${等待秒数} 秒。`),
        });
      } catch (错误) {
        if (!是抖音无待开票订单错误(错误)) throw 错误;
        const message = `已跳过：${店铺配置.name} 当前没有待回传订单。`;
        通知回传进度(onProgress, 构建阶段进度(message));
        return 构建无待回传订单结果(店铺配置, message);
      }
      打印日志('抖音发票回传', '导出订单', `已导出待回传订单：${exportFilePath}`);
    }
    通知回传进度(onProgress, 构建阶段进度(
      使用本地已登记订单 ? '正在读取本地发票已登记订单。' : '正在读取抖音待回传订单报表。',
    ));
    const orders = 使用本地已登记订单
      ? [...指定订单列表]
      : 读取抖音导出订单(exportFilePath, 店铺配置);
    if (!orders.length) {
      const message = '已跳过：导出报表没有可回传订单。';
      打印日志('抖音发票回传', '读取订单', message);
      return {
        ...构建无待回传订单结果(店铺配置, message),
        exportFilePath,
      };
    }
    通知回传进度(onProgress, {
      type: 'orders',
      status: 'running',
      message: `已读取抖音待回传订单 ${orders.length} 单。`,
      exportFilePath,
    ...
    const message = `抖音发票回传完成：成功 ${successCount}/${orders.length} 单，失败或跳过 ${failedCount} 单。`;
    return {
      message,
      exportFilePath,
...
async function 执行抖音发票正式回传(选项 = {}) {
  // 解决：正式入口只消费本地已同步待回传队列（抖音没有“发票已登记”人工阶段），逐单留痕并提交。
  const gate = 创建正式回传闸门({
    store: 选项.店铺配置,
    orders: 选项.orders,
  ...
  return 执行受控正式回传({
    ...
    store: 选项.店铺配置,
    gate,
    execute: (orders, onProgress) => (选项.执行回传方法 || 执行抖音发票回传)({ ...选项, orders, onProgress }),
  });
}
