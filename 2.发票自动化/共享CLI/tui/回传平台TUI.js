  ...
  const 页面 = {
    ...
    render(app) {
      ...
      if (可见列表.length === 0) {
        ...
        if (全部列表.length === 0) {
          // 空列表最容易看成"没同步过"，所以把本地数据最后更新时间直接写在下面（用户 2026-09-14 反馈）。
          const 数据更新时间 = 读取文件更新时间(模板选项.数据更新时间文件);
          行列表.push(着色(适配宽度(
            数据更新时间
              ? `（本地数据最后更新：${数据更新时间}；平台没有待处理订单时这里就是空的）`
              : "（还没同步过订单数据，请先到总览页跑一次同步）",
            列数,
          ), "gray"));
        } else {
          行列表.push(着色(适配宽度(`（当前过滤「${过滤模式.label}」下没有订单，按 f 切换）`, 列数), "gray"));
        }
        return 行列表;
      }
