...
// 默认看「全部」：否则订单都已安排/已处理时列表看起来像空的，分不清"还没处理"还是"没有数据"（用户 2026-09-14 要求所有项目统一）。
const 默认订单过滤模式索引 = Math.max(0, 订单过滤模式.findIndex((模式) => 模式.id === "all"));

function 过滤订单镜像列表(镜像列表, 模式索引 = 默认订单过滤模式索引) {
  const 模式 = 订单过滤模式[模式索引] || 订单过滤模式[默认订单过滤模式索引];
