...
function 读取下载中心地址(选项 = {}) {
  // 解决：下载中心地址集中配置，拼多多项目只依赖 HTTP 契约。
...
function 启动下载中心服务(projectRoot) {
  ...
  child.unref();
  打印日志('拼多多发票回传', '下载中心', `已拉起通用发票下载中心：PID=${child.pid}`);
  return child.pid;
}
...
function 规范化下载中心订单列表(orders) {
  // 解决：只把下载中心需要的订单身份传过去，不把拼多多上传细节带入公共服务。
  ...
  if (!订单列表.length) {
    throw new Error('下载中心下载发票失败：没有可下载的拼多多订单。');
  }
  return 订单列表;
}

function 合并下载结果与订单(订单列表, files) {
  // 解决：下载中心按订单号返回文件和发票号码，拼多多流程继续保留自己的订单身份。
  ...
  return 订单列表.map((order) => {
    ...
    return {
      ...
      invoiceBuyerName: String(file.invoiceBuyerName || '').trim(),
...
async function 批量从下载中心下载发票({ orders, baseUrl, projectRoot, autoStart = true, force = false, startTimeoutMs, requestTimeoutMs = 3 * 60_000 } = {}) {
  // 解决：拼多多项目只提交订单号并拿回发票路径，下载实现完全交给公共下载中心。
  const 订单列表 = 规范化下载中心订单列表(orders);
  const 服务地址 = await 确保下载中心可用({ baseUrl, projectRoot, autoStart, startTimeoutMs });
  打印日志('拼多多发票回传', '下载中心', `请求下载发票：${订单列表.length} 单`);
  const response = await 发送下载中心请求(构建接口地址(服务地址, '/api/invoices/download'), {
    method: 'POST',
    payload: { orders: 订单列表, force: force === true },
