...
const { 登录首个或指定拼多多店铺, 登录全部启用拼多多店铺 } = require('./app/loginStores');
const { 采集首个或指定拼多多店铺元素 } = require('./app/collectPddElements');
...
async function main() {
  ...
  if (模式 === 'login') {
    ...
    await 登录首个或指定拼多多店铺(storeId, { headless: false });
    return;
  }
  if (模式 === 'login-all') {
    await 登录全部启用拼多多店铺({ headless: false });
    return;
  }
  if (模式 === 'collect') {
    const storeId = process.argv[3] || '';
    await 采集首个或指定拼多多店铺元素(storeId, { headless: false });
  }
}
