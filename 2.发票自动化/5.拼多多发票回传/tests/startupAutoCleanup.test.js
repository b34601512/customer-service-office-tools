...
function 创建临时项目目录() {
  ...
  return fs.mkdtempSync(path.join(os.tmpdir(), 'pdd-runtime-cleanup-'));
}

test('启动清理路径不包含登录资料和旧控制台目录', () => {
  const projectRoot = path.join(os.tmpdir(), 'pdd-project');
  const paths = 构建启动清理路径列表(projectRoot);

  assert.equal(paths.some((item) => item.includes(path.join('runtime', 'store-profiles'))), false);
  assert.equal(paths.some((item) => item.includes(path.join('runtime', 'pdd-exports'))), true);
