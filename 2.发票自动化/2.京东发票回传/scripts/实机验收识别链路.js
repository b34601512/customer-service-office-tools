...
// ① 进度回调真实逐页推进 ② 店铺快照与订单记录被本轮刷新 ③ 页面标题/地址来自真实京东页面
...
async function main() {
  ...
  // ③ 真实页面：标题与地址来自真实京东页面（2026-09-16 起不再保存页面截图，凭证只保留文本）
  断言('页面标题来自真实京东页面', /京东|京麦/.test(String(结果.pageTitle || '')), `标题=${结果.pageTitle || '空'}`);
  断言('页面地址是京东后台', /jd.com|jingdong/.test(String(结果.pageUrl || '')), `地址=${结果.pageUrl || '空'}`);
  断言('失败现场不再产生截图路径', !结果.screenshotPath, `截图路径=${结果.screenshotPath || '无'}`);
