...
function 记录清理摘要({ projectRoot, now, 清理类型, 日志模块名, 路径列表, 清理结果 }) {
  // 解决：每次自动清理都记录摘要，性能面板能解释本次启动到底做了什么。
  记录自动清理结果({
    projectRoot,
    now,
    cleanupType: 清理类型,
    moduleName: 日志模块名,
    checkedPathCount: 路径列表.length,
    thresholdBytes: 0,
    cleanupResults: 清理结果,
  });
}
...
function 打印清理日志(日志模块名, 清理结果) {
  ...
  打印日志('运行目录清理', 日志模块名, `已迁移垃圾=${清理结果.length} 项，约 ${格式化体积(总字节数)}`);
}

function 清理运行垃圾路径列表({ 路径列表, 日志模块名, 清理类型, now = new Date(), projectRoot = 项目根目录, 备份根目录 }) {
  // 解决：统一编排运行垃圾迁移、摘要记录和日志输出。
