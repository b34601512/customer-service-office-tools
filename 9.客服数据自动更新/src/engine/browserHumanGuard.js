// 只识别登录/人工验证信号，不破解验证码、不点击授权/确认/开通按钮。
const {
  getAutomationScope, assertAutomationActive, requireHeadedBrowser
...
function isLoginUrl(rawUrl, platformKey) {
  ...
  const url = new URL(rawUrl);
  return /(^|\.)(login|passport|auth)\./i.test(url.hostname) || /\/(login|passport)(\/|\.|$)/i.test(url.pathname);
}
...
function isTransientPageError(error) {
  return /Execution context was destroyed|Frame was detached|frame has been detached|Target page, context or browser has been closed/i.test(String(error?.message || error));
}
...
async function detectHumanRequirement(page, options = {}) {
  ...
  for (const surface of surfaces) {
    try {
      if (await anyVisible(surface.getByText(CHALLENGE_TEXT))) return "页面要求人工安全验证";
      if (options.includeLogin && (
        await anyVisible(surface.locator("input[type='password']")) ||
        await anyVisible(surface.getByText(LOGIN_TEXT))
...
async function scanBrowserForHumanRequirement(scope) {
  const browser = scope.browser;
  if (!browser) return;
  for (const context of browser.contexts()) {
    for (const page of context.pages()) {
      const options = { platformKey: scope.platformKey, includeLogin: scope.headless };
      const reason = await detectHumanRequirement(page, options);
      if (!reason) continue;
      requireHeadedBrowser(reason);
      await waitForHumanResolution(page, reason, () => detectHumanRequirement(page, options));
    }
  }
}
...
async function checkBrowserHumanRequirement(options = {}) {
  ...
  scope.guardPromise = scanBrowserForHumanRequirement(scope);
  try { await scope.guardPromise; }
  finally { scope.guardPromise = null; }
}

module.exports = {
  CHALLENGE_TEXT, PLATFORM_HOSTS, isPlatformUrl, isLoginUrl,
  anyVisible, detectHumanRequirement, checkBrowserHumanRequirement, waitForHumanResolution
};
