...
function buildStoreBrowserCacheRoots(runtimeConfig = appConfig) {
  // 这个函数只返回全部业务店铺浏览器资料根目录，只适合启动前、退出后等低频全量清理。
  return [
    runtimeConfig.storeChromeProfilesRoot,
    runtimeConfig.chromeUserDataDir
  ];
...
function cleanRuntimeBrowserCachesWhenSafe(triggerName = "浏览器缓存自动清理") {
  // 这个函数只在全部受控浏览器都关闭后清理所有浏览器缓存。
  return cleanRuntimeBrowserCachesWithDependencies({
    triggerName,
    guardPidPaths: [appConfig.chromePidPath],
    profileRoots: buildDefaultRuntimeBrowserCacheRoots(appConfig)
  });
}

function cleanStoreBrowserCachesWhenSafe(triggerName = "店铺浏览器缓存自动清理") {
  // 这个函数只在受控登录浏览器关闭后做低频全量清理，不放进单店开窗日常路径。
  return cleanRuntimeBrowserCachesWithDependencies({
    triggerName,
    guardPidPaths: [appConfig.chromePidPath],
    profileRoots: buildStoreBrowserCacheRoots(appConfig)
  });
}
