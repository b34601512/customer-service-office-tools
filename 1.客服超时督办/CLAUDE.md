...
这是给客服主管使用的本地督办工具。它登录小蟹客服工作台后，持续检查客户是否长期没有得到人工实质回复、是否发生客服转接、该上班时段是否无人在线，并在非上班时间按排班自动关闭「自动分配/可被转接」，避免客户转给未到岗客服后无人回复；客户超时提醒时，如果原接待已经不在值班时段，会按排班把客户自动转给当前当班且在线的同组客服（售前转售前、售后转售后，运营客户按售前处理）；需要提醒时发送到对应企微群，处理记录和绩效事实留在本地。
...
- `src/features/timeoutAutoTransfer/`：超时自动转接规则层。原接待不在班（含运营账号）时，按“同组、当班班次、有值班背景色、在线开关开着”挑人转接；跨组不转（售前处理不了售后）。
