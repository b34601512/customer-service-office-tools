...
# 2026-09-30 在线实时值（主管 9/20 改过：徐佳楠 9/30 休、麦诺谦 9/30 早）
SEP = {
    "韩欢欢": ("早", 1), "麦诺谦": ("早", 7), "叶炳辉": ("晚", 3),
    "徐佳楠": ("休", 0), "刘秀文": ("晚", 4),
...
# 10 月固定需求（在线表浅蓝标记）：麦诺谦 1-3 休、刘秀文 4 休 + 23-25 休
# 冲突取舍：叶炳辉最晚 4 号休（刘秀文已占 4 号）→ 改 5 号休；刘秀文 5 号上班。
# 两人跨月连上 7 天（叶 9/28-10/4、刘 9/27-10/3）：超 6 天上限，验收需说明。
SELLER_REST = {
    "韩欢欢": [6, 12, 18, 19, 26, 31],
    "麦诺谦": [1, 2, 3, 8, 13, 20, 27],
    "叶炳辉": [5, 9, 10, 16, 21, 28],
    "徐佳楠": [7, 14, 15, 22, 29, 30],
    "刘秀文": [4, 11, 17, 23, 24, 25],
}
...
AFTER_OFF = {
    "缪婷婷": [6, 8, 9, 15, 22, 29],           # 实休 6 天（可休 5 天 → 超休 1 天，记作休下个月的假）
    "柯紫婷": [4, 11, 18, 20, 27, 28],           # 实休 6 天（可休 11 天）
    "邓远祥": [2, 3, 10, 17, 24, 26],           # 实休 6 天（可休 6天1小时）
    "陈燕玲": [5, 12, 14, 16, 21, 23, 30],      # 实休 7 天（可休 7天3小时）
}
