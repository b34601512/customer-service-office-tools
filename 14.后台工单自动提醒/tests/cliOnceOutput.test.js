...
// 于是终端只看到 `undefined` —— 字段真源写错就是这个后果。本测试锁死：终端输出必须打真实文案。
test("巡检输出必须打印真实文案（messages 数组），不允许出现 undefined", () => {
  const lines = formatOnceResult({
    sent: [
      {
        event: { sourceId: "jd/jingxi2/workorder", type: "login_required" },
        messages: ["【工单提醒】京喜2店 后台工单\n店铺登录态已失效，请尽快重新登录京麦。"],
        ok: true
      }
    ]
  });
  const 全文 = lines.join("\n");
  assert.match(全文, /已发送/, "成功发送要当场吐状态");
...
test("一单一消息：多条消息要逐条打印", () => {
  const lines = formatOnceResult({
    sent: [{ event: { sourceId: "jd/jd3/workorder", type: "count_increase" }, messages: ["单1文案", "单2文案"], ok: true }]
  });
...
test("发送失败要显示原因；没有变化要明说（不许静默）", () => {
  ...
  assert.match(formatOnceResult({ sent: [{ event: { sourceId: "a/b/c", type: "t" }, messages: [], ok: true }] }).join("\n"), /没有生成文案/);
});
