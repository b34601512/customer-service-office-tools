// 常驻监控的浏览器池：一个店一个窗口、每轮复用、窗口没了才重新拉起、收工不关窗口。
// 运行目录用 WORK_ORDER_HOME 隔离到临时目录，不碰正式 runtime。
...
test("同一个店铺第二轮复用同一个窗口，不再开新窗口", async () => {
  ...
  const pool = createStoreBrowserPool({ openImpl: makeOpenImpl(state) });
...
test("多个店铺各占一个调试端口，并把端口登记到 browser-ports.json", async () => {
  ...
  const ports = state.calls.map((item) => item.reusePort);
...
test("重启后按登记表附着回原端口（reusePort 用登记值，不另开端口）", async () => {
  ...
  await pool.ensure("jd", store);
  const firstPort = state.calls[0].reusePort;

  const state2 = { calls: [], alive: true, session: null };
  const pool2 = createStoreBrowserPool({ openImpl: makeOpenImpl(state2) }); // 新进程
  await pool2.ensure("jd", store);

  assert.strictEqual(state2.calls[0].reusePort, firstPort, "新进程必须优先附着登记表里的端口");
});
