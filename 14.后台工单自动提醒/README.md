...
后台出现需处理的工单时自动提醒企业微信群：先支持京东两类店铺——京喜店「任务工单」（sale-jdm.jd.com）与 POP 店「交易纠纷」（shop.jd.com），多店铺各自独立登录态；店铺登录失效也会提醒。提醒附新增单**订单号**；纠纷**判责未出每 30 分钟重发（可配），判责出了只提醒一次**。只@**当日值班售后**（组长在班@组长，其他人看金山排班表背景标记色）+ 主管。详见 `CLAUDE.md`、GitHub issue #623（元素采集）/#624（设计验收）/#631（值班@与底色）。
