...
from refund_reminder.web_control import control_center_window_lifecycle as lifecycle
from refund_reminder.web_control.server import _open_control_center_browser
...
class ControlCenterWindowLifecycleTest(unittest.TestCase):
    def test_watchdog_command_is_python_with_fixed_pids(self) -> None:
        command = lifecycle._build_cleanup_watchdog_command(  # noqa: SLF001
            ...
            shutdown_url="http://127.0.0.1:39480/api/control/exit",
        )
    ...
    def test_server_passes_spawned_browser_pid_to_watchdog(self) -> None:
        service = Mock()
        service.config.login.browser_executable = ""
        process = Mock(pid=4321)

        with tempfile.TemporaryDirectory() as temp_dir:
            root_dir = Path(temp_dir)
            with (
                patch("refund_reminder.web_control.server.resolve_browser_executable", return_value="chrome.exe"),
                patch("refund_reminder.web_control.server.close_browser_processes_by_profile"),
                patch("refund_reminder.web_control.server.clean_browser_profile_cache"),
                patch("refund_reminder.web_control.server.subprocess.Popen", return_value=process),
                patch("refund_reminder.web_control.server.start_control_center_cleanup_watchdog") as start_watchdog,
            ):
                handle = _open_control_center_browser(service=service, root_dir=root_dir, url="http://127.0.0.1:39480")

        self.assertEqual(handle.process_id, 4321)
        start_watchdog.assert_called_once_with(handle, shutdown_url="http://127.0.0.1:39480/api/control/exit")
