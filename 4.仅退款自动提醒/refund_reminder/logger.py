...
def _caller_location(stacklevel: int) -> str:
    # 该函数用于把日志定位到真实调用方，避免排查时只能看到日志模块自己。
    try:
...
def log(main: str, main_action_name: str, module: str, sub_action: str, **kwargs: Any) -> None:
    # 该函数用于输出统一格式终端日志，方便从主线动作一路追到具体模块。
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    location = _caller_location(2)
    main_action = f"{main}:{main_action_name}" if str(main_action_name or "").strip() else str(main or "")
