...
def candidate_browsers(configured: str) -> list[str]:
    # 该函数用于寻找 Playwright 可直接控制的 Chrome/Edge 浏览器路径。
    out: list[str] = []
...
def resolve_browser_executable(configured: str) -> str:
    # 该函数用于显式定位浏览器，找不到就直接失败并暴露根因。
    for exe in candidate_browsers(configured):
