...
## 业务口径（用户 2026-09-17 定稿）

- **需要安排客服回访的 = 只看「排队阶段」呼损**（客户进了排队但没等到人工就挂了）。
- **IVR 阶段呼损只做参考**；唯一例外：**同一客户在统计窗口内呼入 ≥10 次**才需要关注
  —— 怀疑客户不知道要按数字键才能转人工，反复打都进不去。
  - 「呼入次数」口径 = 呼入表（已接通）行数 **+** 呼损表（未接通）行数，按呼入时间去重。
  - 实现：`scripts/query_queue_loss.py`（`--ivr-threshold`，默认 10，含 10）；报告落在
    `电话漏接分析后台/runtime/queue_loss_report_<日期>.txt`。
