...
这份项目教AI如何像工作秘书一样：理解主管意图、读取资料、分析业务规则、逐项请主管拍板、配置后台并核验。默认适配探域后台；使用其他系统时复用工作方法，但必须重新确认接口和字段。
...
| 模板/ | 空白接入及拍板记录 | 可以 |
| 通用经验/ | 在线文档读取、探域API/无头执行方法和参数化模板 | 可以 |
| 工具/ | 按白名单制作通用分享包 | 可以 |
...
接收方解压后从 `通用经验/通用AI入口.md` 开始，用自己的公司资料和登录环境。通用包只包含通用方法、探域平台适配和参数化API模板，不含任何公司业务数据：AI需要按手册验证当前后台后实施。没有这些能力时先输出草稿交主管手动配置，不能假装已经操作。
