...
> 目标: 德达医疗旗舰店(淘宝)客服机器人自动回复。先读本手册 + `README.md` + `审核报告.md`, 不要重新摸索。

## 0. 铁律
- 独立浏览器画像固定用 `C:/Users/b3460/.pi-edge-auto`, 永远 headless:false, 不要关闭窗口, 登录态只存本机。
...
await page.goto('http://agent.tanyuai.com/v2/dashboard', { waitUntil: 'domcontentloaded' }).catch(()=>{});
await page.waitForTimeout(5000); // 等登录态 + 前端加载
```
- 首次无登录态时用有头窗口人工扫码/登录一次, 以后自动记住。
...
```
- 店铺: 德达医疗旗舰店, groupId=2094743970688012288, thirdShopId=2095398963959042048, chatbotShopId=2095399454105407488, platform=1(淘宝), sellerId=706231158。
- 绑定客服: 小黛/德德/嘟嘟/璇璇; 策略: 全自动 + 智能体优先(防抢话)。

## 3. 知识库接口(核心, 都在页面 fetch 里调)
| 操作 | 接口 | 要点 |
|---|---|---|
| 查总数/全拉 | POST `/api/kbe/v1/knowledge-card/page` `{pageNo:1,pageSize:3000}` | 必须 pageSize:3000 一次拉全; pageSize:500 翻页会重复 id; 返回 `data.total` 经常是**过期缓存**, 以 `results.length` 为准 |
| 逐条新增 | POST `/api/kbe/v1/knowledge-card/save` `{title,content:[{content}],labels:[],ifBelievable:true,knowledgeType:'CHAT',ifOpen:true}` | batch-create/batch-save/import 都是 405, 只能逐条, 并发 4 |
| 带 id 保存=原位更新 | 同上 + `id` 字段 | 已验证: 不会产生重复卡, 可用来改错别字/改数字 |
...
## 5. 官方文档
- 飞书《智能体2026新页面后台(实施版)操作文档》: `https://my.feishu.cn/wiki/IANhwmQCio7cxkBdBacnVK5nBh`, 密码 `3189&63s`。
...
## 7. 当前状态(2026-09-04)
- 知识库 2657→2198 条; 超长无标题 48→12; 用户待拍板: 平台专属去留/品牌口径/医疗口径/承诺兑现/时效。
