...
(async () => {
  ...
  await page.locator('button:has-text("请选择")').first().click({ timeout: 8000 });
  await page.waitForTimeout(2500);
  await page.screenshot({ path: 'C:/Users/b3460/.pi-edge-work/bind-kefu.png' });
  const t = await page.evaluate(() => document.body.innerText.slice(-1200));
  console.log('KEFU-DIALOG', JSON.stringify(t).slice(0, 800));
  console.log('DONE');
  await ctx.close();
})();
