...
- `playwright-core` 连接程序拉起的 Google Chrome，并复用隔离的长期登录资料。
...
`读取配置和历史` → `检查表格占用` → `筛选启用店铺` → `复用当天完整结果或打开 Chrome` → `无头读取；必要时有头登录/验证/切店` → `生成带日期、来源和记录键的指标` → `覆盖或追加 Excel` → `保存凭证和历史` → `按需同步金山文档`。单店失败不阻断后续店铺；固定指标无值时写0并标记无数据，采集等待期间由控制中心持续发布动作心跳。

浏览器切换为 Chrome 后，旧浏览器资料不直接混用；首次运行各平台/店铺需在 Chrome 长期资料目录中登录一次，后续复用该资料。
