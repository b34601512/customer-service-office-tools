...
function shouldKeepPddBrowserOpen(error) {
  return /滑块|安全验证|验证码|扫码确认|等待拼多多登录成功超时|等待人工验证/.test(String(error?.message || error));
}
...
async function collectAndWritePddStoreMetrics({ config, store, dateSelection, onProgress }) {
  ...
  try {
    ...
    return await runHybridStoreCollection({
      ...
      try {
        const page = await waitForPddLoginReady(
          browser,
          { ...store, siteUrl: store.sources.customer },
          {
        ...
        if (browser) {
          await disconnectFromChrome(browser, "拼多多店铺指标本轮采集结束，断开自动化连接").catch(() => {});
          browser = null;
        }
      }
    });
