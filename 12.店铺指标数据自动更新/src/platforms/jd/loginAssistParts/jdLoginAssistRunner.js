...
} = require("../loginSurfaceParts/jdLoginSurfaceState");
...
async function processJdAssistPage(page, resolvedConfig, credentials, autofilledSurfaceMarks, options = {}) {
  // 这里处理单个京东辅助页，把主循环从页面内细节中解放出来。
  const displayName = resolvedConfig.activeStore.displayName;
  const url = page.url();
  if (!/passport\.jd\.com|passport\.shop\.jd\.com|xi\.jd\.com|kf\.jd\.com|jdsz\.jd\.com|shop\.jd\.com/i.test(url)) {
    return;
  }
...
async function runJdLoginAssist(assistTask, options = {}) {
  ...
  try {
    ...
    while (Date.now() <= deadline) {
      ...
      const stateStep = await runJdLoginStateStep(async () => {
        ...
        for (const page of collectAssistPages(browser, resolvedConfig.activeStore)) {
          ...
          const pageResult = await processJdAssistPage(
            page,
            resolvedConfig,
            { username, password },
              ...
              onManualVerification(verificationState) {
                if (verificationState.reason !== lastManualVerificationReason) {
                  ...
                  log(
                    ...
                    "京东登录",
                    "人工验证",
                    `店铺「${displayName}」需要${verificationState.reason}，浏览器会停留在原页面等待用户完成`
                  );
                  if (typeof options.onManualVerification === "function") {
                    options.onManualVerification(verificationState);
                  }
                }
              }
            }
          );
          if (pageResult?.waitingForManualVerification) {
            // 用户明确需要操作时持续顺延等待，不关闭窗口、不切换下一家店。
