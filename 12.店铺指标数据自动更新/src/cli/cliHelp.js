...
async function showCliHelp(terminal) {
  ...
  terminal.writeLine("   默认先用无头 Edge 采集；遇到登录、验证码或人工切店时自动切换到可见 Edge。");
  ...
  terminal.writeLine("8. 本程序固定使用 Microsoft Edge，不回退到 Chrome；可用 npm run cli:headed 或 cli:headless 指定模式。");
  terminal.writeLine("9. 本程序运行不需要AI；首页[A]可把本地“数据源”一次同步到金山文档，并回读数量确认结果。");
