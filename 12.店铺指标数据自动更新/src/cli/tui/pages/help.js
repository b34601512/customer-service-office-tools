...
const HELP_LINES = [
  ...
  "   默认先用无头 Edge 采集；遇到登录、验证码或人工切店时自动切换到可见 Edge。",
  "5. 运行前请保存并关闭汇总表；被WPS或Excel占用时会立即停止。",
  "6. 店铺管理可修改编号；新增店铺可直接填6，允许跳过中间编号。",
  "7. 店铺指标只保留平台考核最终数值，不保存客服个人或业务明细。",
  "8. 本程序固定使用 Microsoft Edge，不回退到 Chrome；可用 npm run cli:headed 或 cli:headless 指定模式。",
  "9. 本程序运行不需要AI；金山页可把本地“数据源”一次同步到金山文档，并回读数量确认结果。"
];
