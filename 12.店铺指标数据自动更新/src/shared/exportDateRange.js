...
function validateExportDateAutomationConfig(dateAutomationConfig, fallbackDateAutomationConfig) {
  ...
  return {
    dateRangeDayCount: validateExportDateAutomationInteger(
      dateAutomationConfig?.dateRangeDayCount,
      fallbackConfig.dateRangeDayCount ?? defaultConfig.dateRangeDayCount,
      1,
      "查看时间跨度"
    ),
...
function normalizeLooseDateText(value) {
  // 从任意业务文案里宽松提取 YYYY-M-D 并归一成 YYYY-MM-DD，供各平台页面文本解析共用。
  const matchedDate = String(value || "").match(/(\d{4})[-/.年](\d{1,2})[-/.月](\d{1,2})/);
  if (!matchedDate) return "";
  return `${matchedDate[1]}-${String(matchedDate[2]).padStart(2, "0")}-${String(matchedDate[3]).padStart(2, "0")}`;
}

function shiftDateText(dateText, offsetDays) {
  // 文本日期平移的统一实现，各平台解析器不再各自维护副本。
  if (!/^\d{4}-\d{2}-\d{2}$/.test(String(dateText || ""))) return "";
  return formatDate(shiftDate(parseDateText(dateText), offsetDays));
}
...
function resolveAutomatedExportDateRange(dateAutomationConfig, baseDate = new Date()) {
  ...
  const startDate = shiftDate(endDate, 1 - normalizedConfig.dateRangeDayCount);
  return {
    startDate,
    endDate,
