...
async function findOnlyVisiblePopup(surface, options) {
  // 该函数只确认当前操作面存在唯一遮挡弹窗。
  const dialogSelectors = options.dialogSelectors || [
...
async function runAfterDismissingBlockingPopups(surface, action, options = {}) {
  ...
  const shouldPreserveSurface = async () => {
    if (typeof options.shouldPreserveSurface !== "function") {
      return false;
    }
    return Boolean(await options.shouldPreserveSurface(surface));
  };

  if (await shouldPreserveSurface()) {
    return action();
  }
