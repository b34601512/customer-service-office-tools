...
19. **每个解析只允许说一句话**（2026-09-17 用户要求：“这样可以强迫模型只说最重要的重点”）：一个解析 = 标题 + 原话/建议话术 + **一句解说**；`why`/`要点`/`小贴士` 合起来只能一句（按 。！？～… 断句）；标题与话术引用不计入；写不下就删。自检项「解析解说只允许一句话」超出 warn。详见 SKILL.md 铁律 17。
