...
9. **取数要看含自动回复的全量口径**（2026-09-14 踩坑）：列表/`waiterSession/queryChatLog` 不含机器人（`@im.jd.com`）、欢迎语、商品卡片；必须用会话「查看」→「该用户全部聊天记录」核对，否则会把「机器人已答、客服只发了个表情」误判成客服不答问题、**误伤客服**。详见 SKILL.md 铁律 8。
