...
11. **「点击展开」必须一眼可见**（2026-09-15 用户要求）：解析块展开按钮由渲染器统一注入（红色胶囊「点击展开解析／点击收起」+ 轻微呼吸光晕，移动端也不隐藏），AI 不要手写 `sum-hint` 文字提示；自检项「解析块有显而易见的『点击展开』按钮」缺按钮判 fail。详见 SKILL.md 铁律 10。
