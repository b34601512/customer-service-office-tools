...
15. **尽量发现做得好的地方并鼓励**（2026-09-17 用户要求：“尽量发现好的地方就鼓励”）：解析不要全是挑错，一份课件**至少留 1 处“做对了”的正面解析**（不挂 `bad`，写“当时怎么说 + 为什么好 + 可以直接抄的说法”），夸具体动作不空夸；真的没有可夸的不硬编；自检项「至少 1 处“做对了”的正面解析」全是挑错时给 warn（不阻断）。详见 SKILL.md 铁律 13。
