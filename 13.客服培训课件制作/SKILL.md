...
8. **取数必须核对"含自动回复"的全量口径**（2026-09-14 踩坑，务必遵守）：京东聊天记录列表 / `waiterSession/queryChatLog` **只有客服与客户的消息，不含机器人自动回复、欢迎语、商品卡片**。必须点开该会话的「查看」→「该用户全部聊天记录」核对（数据来自 `api.m.jd.com/client.action` 的 `functionId=queryLastLogs`，里面有 `@im.jd.com` 机器人消息）。
   - 否则会把「机器人已经答完、客服只是发个表情打招呼」误判成「客服只回了个表情不回答问题」，**误伤客服**；也可能把机器人自动生成/引导的提问当成客户真实提问。
   - 判断任何一条客服回复好不好之前，先确认看的是含机器人的全量记录。
