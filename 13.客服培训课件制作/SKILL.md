...
13. **尽量发现做得好的地方，就鼓励**（2026-09-17 用户要求：别光挑错）：一份课件里**至少留 1 处“做对了”的正面解析**——客服看完要能学到“标准长什么样”，也要知道自己做对的地方被看见了。
   - 正面解析的写法：**当时怎么说的（原话）+ 为什么好 + 可以直接抄的说法/动作**，跟可优化解析同一套结构，只是不挂 `bad`。
   - 夸要夸**具体动作/话术**（例：“发图让客户看流量表，这招很聪明——多数客户说不清自己用多少氧”），不空夸（“服务态度好”）、不夸到人身上。
   - **别硬编**：整场确实没有可夸的就不硬找；自检项「至少 1 处“做对了”的正面解析」全是挑错时给 warn（不阻断，但要在交付时说明原因）。
   - 解析块顺序：正面解析同样挂在**客服那句话之后**（跟在客服原话后面，别跑到客户提问前面）。
