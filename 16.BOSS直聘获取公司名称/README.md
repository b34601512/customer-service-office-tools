...
用户已要求发布 v0.11 供他人使用，现已改为**直接发源码**：对方安装 Python 3.10+ 与依赖后双击“启动BOSS采集.bat”即可。自 2026-09-16 起不再制作免安装包（历史包已移到 `D:\备份文件夹\办公软件产物备份_20260916`），`build_release.ps1`、`一键打包.bat` 已删。
京东完整字段仍待真实登录验收，不因不再打包而宣称该项完成。以下内容为此前版本记录，其中旧京东入口已经撤下。
