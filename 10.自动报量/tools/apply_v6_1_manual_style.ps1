...
$TargetPath = 'D:\桌面\办公软件\10.自动报量\html导入工具\2026年智能报量-v6.1.xlsx'
$BackupDirectory = 'D:\备份文件夹\自动报量\v6.1样式复用'
$Timestamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$BackupPath = Join-Path $BackupDirectory "2026年智能报量-v6.1-样式复用前-$Timestamp.xlsx"
$DateShiftColumnWidth = 9.0
$DayShiftFillHex = 'EDEDED'
$NightShiftFillHex = 'BDD7EE'
...
function Convert-HexRgbToExcelColor {
  param([string]$HexRgb)
  # Excel COM 使用 OLE_COLOR 整数，这里集中转换避免手写颜色值时写反。
  if ($HexRgb -notmatch '^[0-9A-Fa-f]{6}$') { throw "非法RGB颜色：$HexRgb" }
  $red = [Convert]::ToInt32($HexRgb.Substring(0, 2), 16)
  $green = [Convert]::ToInt32($HexRgb.Substring(2, 2), 16)
  $blue = [Convert]::ToInt32($HexRgb.Substring(4, 2), 16)
...
function Apply-ShiftColumnFills {
  param(
    [object]$TargetSheet,
    [int]$DayCount,
    [int]$LastRow,
  ...
  # 统一白班/夜班颜色：白班浅灰、夜班浅蓝，跳过合并日期标题避免破坏日期块。
  $dayFillColor = Convert-HexRgbToExcelColor $DayShiftFillHex
  $nightFillColor = Convert-HexRgbToExcelColor $NightShiftFillHex
  $mergedRowSet = @{}
  foreach ($mergedRow in $MergedRows) {
    $mergedRowSet[[int]$mergedRow] = $true
  }

  $rowSegments = New-Object System.Collections.Generic.List[object]
  $segmentStart = $null
  for ($row = 4; $row -le $LastRow; $row++) {
    if ($mergedRowSet.ContainsKey($row)) {
      if ($segmentStart -ne $null) {
        $rowSegments.Add(@([int]$segmentStart, [int]($row - 1)))
        $segmentStart = $null
      }
      continue
    }
    if ($segmentStart -eq $null) {
      $segmentStart = $row
    }
  }
  if ($segmentStart -ne $null) {
    $rowSegments.Add(@([int]$segmentStart, [int]$LastRow))
  }
  ...
  for ($day = 1; $day -le $DayCount; $day++) {
    ...
    $dayColumnName = Get-ExcelColumnName $targetDayColumn
    $nightColumnName = Get-ExcelColumnName $targetNightColumn
    foreach ($segment in $rowSegments) {
      $startRow = $segment[0]
      $endRow = $segment[1]
      $dayRange = $TargetSheet.Range("${dayColumnName}${startRow}:${dayColumnName}${endRow}")
      $nightRange = $TargetSheet.Range("${nightColumnName}${startRow}:${nightColumnName}${endRow}")
      $dayRange.Interior.Pattern = 1
      $dayRange.Interior.Color = $dayFillColor
      $nightRange.Interior.Pattern = 1
      $nightRange.Interior.Color = $nightFillColor
    }
  }
}

Write-StyleLog 'apply_v6_1_manual_style.ps1:68' '样式复用' '备份' '创建备份目录'
New-Item -ItemType Directory -Force -Path $BackupDirectory | Out-Null
Copy-Item -LiteralPath $TargetPath -Destination $BackupPath -Force
Write-StyleLog 'apply_v6_1_manual_style.ps1:71' '样式复用' '备份' "已备份到 $BackupPath"
...
try {
  Write-StyleLog 'apply_v6_1_manual_style.ps1:81' '样式复用' 'Excel' '打开手工表和v6.1模板'
  ...
  foreach ($targetSheet in $targetWorkbook.Worksheets) {
    ...
    Write-StyleLog 'apply_v6_1_manual_style.ps1:95' '样式复用' $sheetName '复制基础列格式'
    ...
    Write-StyleLog 'apply_v6_1_manual_style.ps1:108' '样式复用' $sheetName '复制日期列格式'
    for ($day = 1; $day -le $dayCount; $day++) {
    ...
    Apply-ShiftColumnFills $targetSheet $dayCount $lastRow $storeHeaderRows
  ...
  Write-StyleLog 'apply_v6_1_manual_style.ps1:128' '样式复用' 'Excel' '保存v6.1模板'
  $targetWorkbook.Save()
}
finally {
...
Write-StyleLog 'apply_v6_1_manual_style.ps1:138' '样式复用' '完成' '手工表样式已应用到v6.1模板'
