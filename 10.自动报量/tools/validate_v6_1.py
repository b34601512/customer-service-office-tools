...
WORKBOOK_PATH = HTML_TOOL_DIR / "2026年智能报量-v6.1.xlsx"
# 本文件与 tools/validate_v6_2.py 为近逐字镜像（仅目标工作簿/QA路径与班次颜色校验差异），
...
QA_PATH = HTML_TOOL_DIR / "v6_1_validation_qa.json"
...
def log(action: str, module: str, sub_action: str) -> None:
    print(f"[validate_v6_1.py][主线:{action}][{module}][{sub_action}]")
...
def expected_blocks(manual: dict) -> list[dict]:
    # 从手工结构里还原店铺块，验证 v6.1 没有漏掉店铺标题和汇总行。
...
def is_expected_fill(cell, expected_color: str) -> bool:
    # 班次颜色必须用显式RGB，避免主题色在不同电脑或WPS里显示漂移。
    color = cell.fill.fgColor
    return cell.fill.patternType == "solid" and color.type == "rgb" and color.rgb == expected_color
...
def validate() -> dict:
    ...
    for month in range(1, 13):
        ...
        for row_number in range(4, manual["rows"] + 1):
            for day in range(1, days + 1):
                if is_merged_pair(worksheet, row_number, day_col(day), night_col(day)):
                    continue
                day_cell = worksheet.cell(row_number, day_col(day))
                night_cell = worksheet.cell(row_number, night_col(day))
                if isinstance(day_cell, MergedCell) or isinstance(night_cell, MergedCell):
                    continue
                if not is_expected_fill(day_cell, SHIFT_DAY_FILL_COLOR):
                    errors.append(f"{sheet_name} R{row_number} 白班颜色异常：{day_cell.coordinate}")
                    break
                if not is_expected_fill(night_cell, SHIFT_NIGHT_FILL_COLOR):
                    errors.append(f"{sheet_name} R{row_number} 夜班颜色异常：{night_cell.coordinate}")
                    break
